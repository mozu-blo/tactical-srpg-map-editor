...
- Height数字の表記（0 / 0.5 / 1 / 1.5 / 7）
- カメラ水平移動の方向、Yaw 0°/90°、Zoom 2倍、Pitch 42°/90°での計算
...
- Height表示：上面への数字表示、変更直後の更新、表示OFFで一括非表示：PASS
- 真上およびPitch 42°の4方向戦闘視点で数字表示：PASS
- カメラ欄の2本指ドラッグ切替、中央に戻す、既存プリセット表示：PASS
- 新しい表示設定はJSONのMap Dataに追加しない：コード確認PASS
- PC中ドラッグ／Shift＋右ドラッグの実入力はComputer Use環境で再現できず、計算テストとコード確認まで
...
- Height数字：390×844の16×16表示、メニュー折りたたみ：PASS
- 390×844で初期盤面の左右端が切れずに収まること：PASS
- 390×844でセル編集と戻る：PASS
- 2本指Pan／ピンチの実入力はComputer Use環境で再現できず、実機確認へ継続
- Gameplay HeightとVisual Yを分離し、RenderHeight = GameplayHeight × 0.4：PASS
- H3/H5の描画差0.8、H3.5/H7の描画差1.4の換算テスト：PASS
- JSONのHeight値、編集量、Undo/Redo、Import/Export、保存データ：変更なし
- Service Workerキャッシュ番号を更新し、既存PWAへ新しい描画コードを配信：PASS

## iPhone実機で確認が必要

- Safariでホーム画面へ追加後の起動
- 2本指カメラとピンチの実機感度
- 2本指Panとピンチの同時操作、拡大後の端セル編集
- 長いスワイプ連続編集の追従
